								...
								if handcuffed then
									table.insert(seized_guns, "**"..cop_name.." "..cop_lname.." ("..user_id..")** beslaglagde følgende våben fra **"..civ_name.." "..civ_lname.." ("..nuser_id..")**: \n")
									  vRPclient.replaceWeapons(nplayer,{},function(weapons)
										...
										--PerformHttpRequest('https://discord.com/api/webhooks/804281559628644413/GEyKkOWzLnRTvTV6JXCDdWHfkZA5uhJs3Us5jx3aq-Fxr0CS2mIj-tofEXI_n6FW6sJ_', function(err, text, headers) end, 'POST', json.encode({username = "ServerNavn [POLITI]" .. GetConvar("servernumber","#1"), content = table.concat(seized_guns)}), { ['Content-Type'] = 'application/json' })
						...
						vRP.getUserIdentity(nuser_id, function(identity_civ)
							if identity_cop and identity_civ then
								...
								if handcuffed then
									...
									--PerformHttpRequest('https://discordapp.com/api/webhooks/640568552356577328/wmPBS04iWFNPFb6xcYj88v5g6kq91Kavngga8N8U9Zkab2_XBCxW1jP6Ki-gesKwCm4Q', function(err, text, headers) end, 'POST', json.encode({username = "ServerNavn [POLITI]" .. GetConvar("servernumber","#1"), content = table.concat(seized_items)}), { ['Content-Type'] = 'application/json' })
		...
		vRPclient.replaceWeapons(player,{},function(weapons)
			vRP.getUserIdentity(user_id, function(identity)
				...
				if identity then
					...
					--PerformHttpRequest('https://discord.com/api/webhooks/804281559628644413/GEyKkOWzLnRTvTV6JXCDdWHfkZA5uhJs3Us5jx3aq-Fxr0CS2mIj-tofEXI_n6FW6sJ_', function(err, text, headers) end, 'POST', json.encode({username = "ServerNavn [POLITI]" .. GetConvar("servernumber","#1"), content = table.concat(stored_items)}), { ['Content-Type'] = 'application/json' })
					-- clear all weapons
