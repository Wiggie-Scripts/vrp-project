	...
	local isEngineOn = Citizen.InvokeNative(0xAE31E7DF9B5B132E, GetVehiclePedIsIn(GetPlayerPed(-1)))
		...
		if isEngineOn then
			if isRadarExtended then
				...
				DrawAdvancedText(0.1550, 0.7890, -0.016, 0.0028, 0.40,round(percent,1), 255, 255, 255, 255, 0, 1)
			elseif(hud_form == 1) then
				local percent = (essence/0.145)*100
				DrawAdvancedText(0.1650, 0.7890, -0.016, 0.0028, 0.40,round(percent,1), 255, 255, 255, 255, 0, 1)
			else
				local percent = (essence/0.145)*100
			end
			DrawAdvancedText(0.1335, 0.7890, -0.016, 0.0028, 0.40,"BENZIN ", 255, 255, 255, 255, 0, 1)
			DrawAdvancedText(0.1825, 0.7890, -0.016, 0.0028, 0.40,"%", 255, 255, 255, 255, 0, 1)
		end
    end
end
