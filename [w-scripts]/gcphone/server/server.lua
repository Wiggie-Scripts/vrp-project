...
MySQL.createCommand("vRP/gcphone_getMessages","SELECT * FROM phone_messages WHERE receiver = @phone")
MySQL.createCommand("vRP/gcphone_getUserIdByPhone","SELECT user_id FROM vrp_user_identities WHERE phone = @phone")
MySQL.createCommand("vRP/gcphone_insertMessage","INSERT INTO phone_messages (transmitter, receiver, message, isRead,owner) VALUES(@transmitter, @receiver, @message, @isRead, @owner); SELECT * from phone_messages WHERE id = (SELECT LAST_INSERT_ID());")
MySQL.createCommand("vRP/gcphone_setRead","UPDATE phone_messages SET phone_messages.isRead = 1 WHERE phone_messages.receiver = @receiver AND phone_messages.transmitter = @transmitter")
MySQL.createCommand("vRP/gcphone_deleteMessage","DELETE FROM phone_messages WHERE `id` = @id")
MySQL.createCommand("vRP/gcphone_deleteAllMessageFromPhoneNumber","DELETE FROM phone_messages WHERE receiver = @mePhoneNumber and transmitter = @phone")
MySQL.createCommand("vRP/gcphone_deleteAllMessage","DELETE FROM phone_messages WHERE receiver = @mePhoneNumber")
MySQL.createCommand("vRP/gcphone_getHistoriqueCall","SELECT * FROM phone_calls WHERE phone_calls.owner = @num ORDER BY time DESC LIMIT 100")
MySQL.createCommand("vRP/gcphone_insertCall","INSERT INTO phone_calls (`owner`, `num`,`incoming`, `accepts`) VALUES(@owner, @num, @incoming, @accepts)")
MySQL.createCommand("vRP/gcphone_deleteCallsFromNumber","DELETE FROM phone_calls WHERE `owner` = @owner AND `num` = @num")
MySQL.createCommand("vRP/gcphone_deleteAllCalls","DELETE FROM phone_calls WHERE `owner` = @owner")
MySQL.createCommand("vRP/gcphone_getTweets","SELECT name,message,time FROM `phone_twitter` ORDER by id DESC LIMIT 100")
MySQL.createCommand("vRP/gcphone_deleteOldMessages","DELETE FROM phone_messages WHERE (DATEDIFF(CURRENT_DATE,time) > 20)")
MySQL.createCommand("vRP/gcphone_deleteOldCalls","DELETE FROM phone_calls WHERE (DATEDIFF(CURRENT_DATE,time) > 20)")
MySQL.createCommand("vRP/gcphone_deleteOldTweets","DELETE FROM phone_calls WHERE (DATEDIFF(CURRENT_DATE,time) > 20)")
MySQL.createCommand("vRP/gcphone_deleteServiceMessages","DELETE FROM phone_messages WHERE (`transmitter` like '%-Job' or `transmitter` = 'Taxi') AND (DATEDIFF(CURRENT_DATE,time) > 1)")
...
--  Utils
--====================================================================================
MySQL.execute("vRP/gcphone_deleteOldMessages") -- Wipe beskeder der er ældre end 31 dage
MySQL.execute("vRP/gcphone_deleteOldCalls") -- Wipe beskeder der er ældre end 31 dage
MySQL.execute("vRP/gcphone_deleteServiceMessages") -- Wipe beskeder der er ældre end 31 dage
...
function _internalAddMessage(transmitter, receiver, message, owner, cbr)
    local task = Task(cbr)
    MySQL.query("vRP/gcphone_insertMessage", {transmitter = transmitter,receiver = receiver, message = message, isRead = owner, owner = owner}, function(rows, affected)
        if #rows > 0 then
            task({rows[1]})
        end
    end)
    ...
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            ...
            if num ~= nil and mePhoneNumber ~= nil then
                MySQL.execute("vRP/gcphone_setRead", {receiver = mePhoneNumber, transmitter = num})
            end
        end
    end})
end
...
function deleteMessage(msgId)
    MySQL.execute("vRP/gcphone_deleteMessage", {id = msgId})
end
...
function deleteAllMessageFromPhoneNumber(identifier, phone)
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.execute("vRP/gcphone_deleteAllMessageFromPhoneNumber", {mePhoneNumber = mePhoneNumber, phone = phone})
        end
    end})
end
...
function deleteAllMessage(identifier)
    vRP.getUserIdentity({identifier, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.execute("vRP/gcphone_deleteAllMessage", {mePhoneNumber = mePhoneNumber})
        end
    end})
end
...
function getHistoriqueCall (num, cbr)
    local task = Task(cbr)
    MySQL.query("vRP/gcphone_getHistoriqueCall", {num = num}, function(rows, affected)
        if #rows > 0 then
            task({rows})
        end
    end)
...
function saveAppels (appelInfo)
    if appelInfo.extraData == nil or appelInfo.extraData.useNumber == nil then
        MySQL.execute("vRP/gcphone_insertCall", {owner = appelInfo.transmitter_num, num = appelInfo.receiver_num, incoming = 1, accepts = appelInfo.is_accepts}, function(affected)
    ...
    if appelInfo.is_valid == true then
        ...
        MySQL.execute("vRP/gcphone_insertCall", {owner = appelInfo.receiver_num, num = num, incoming = 0, accepts = appelInfo.is_accepts}, function(affected)
...
AddEventHandler('gcPhone:appelsDeleteHistorique', function (numero)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.execute("vRP/gcphone_deleteCallsFromNumber", {owner = mePhoneNumber, num = numero})
        end
    end})
...
function appelsDeleteAllHistorique(srcIdentifier)
    vRP.getUserIdentity({srcIdentifier, function(identity)
        if identity then
            local mePhoneNumber = identity.phone
            MySQL.execute("vRP/gcphone_deleteAllCalls", {owner = mePhoneNumber})
        end
    end})
end
...
AddEventHandler('gcPhone:allUpdate', function(user_id, source)
    ...
    vRP.getUserIdentity({user_id, function(identity)
        if identity then
            ...
            MySQL.query("vRP/gcphone_getMessages", {phone = phone}, function(rows, affected)
                if #rows > 0 then
                    TriggerClientEvent("gcPhone:allMessage", lsource, rows)
                end
            end)
			MySQL.query("vRP/gcphone_getTweets", {}, function(rows, affected)
            if #rows > 0 then
